...
def fact_view():
    df = spark.readStream.table("silver_obt")
    df = df.select("ride_id","distance_miles","duration_minutes","base_fare","distance_fare","time_fare","surge_multiplier","rating","tip_amount","total_fare","base_rate","per_mile","per_minute","payment_method_id","driver_id","passenger_id","vehicle_id","pickup_city_id")
    return df
...
dp.create_auto_cdc_flow(
  target = "fact",
  source = "fact_view",
  keys = ["ride_id","payment_method_id","driver_id","passenger_id","vehicle_id","pickup_city_id"],
  sequence_by = "ride_id",
  stored_as_scd_type = 1,
)
